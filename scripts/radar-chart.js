var RadarChart = {
    ...
    function radar(selection) {
      selection.each(function(data) {
        ...
        var maxValue = Math.max(cfg.maxValue, d3.max(data, function(d) {
          return d3.max(d.langs, function(o){ return o.commits / d.commits; });
        }));

        var allAxis = data[0].langs.map(function(i, j){ return {name: i.name, xOffset: (i.xOffset)?i.xOffset:0, yOffset: (i.yOffset)?i.yOffset:0}; });
        ...
        data.forEach(function(d){
          d.langs.forEach(function(axis, i) {
            axis.x = getHorizontalPosition(i, cfg.w/2, (parseFloat(Math.max(axis.commits / d.commits, 0))/maxValue)*cfg.factor);
            axis.y = getVerticalPosition(i, cfg.h/2, (parseFloat(Math.max(axis.commits / d.commits, 0))/maxValue)*cfg.factor);
          });
        });
        ...
        if(cfg.circles && cfg.radius) {
          ...
          var circle = circleGroups.selectAll('.circle').data(function(datum, i) {
            return datum.langs.map(function(d) { return [d, i]; });
          });
  ...
  draw: function(id, d, options) {
    ...
    d = d.filter(function(d){return d.commits > 1000000});
    d.forEach(function(i) {
        i.langs = i.langs.filter(function(lang){
            return lang.commits > 50000;
        });
    });
